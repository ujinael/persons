...
import { NomenclatureItem, NomenclatureType } from '../../models/nomenclature_item/NomenclatureItem';
...
export const useNomenclatureItemsStore = defineStore('nomenclatureItems', {
  ...
  actions: {
setNomenclatureItem(nomenclatureItem?: NomenclatureItem,categoryID?:string) {
      this.nomenclatureItem = nomenclatureItem ??
        new NomenclatureItem(
          '',
          '',
          '',
          NomenclatureType.PRODUCT,this.category!,undefined,undefined,categoryID
        )
},
async fetchNomenclatureItems(searchStr?:string) {
  this.loading = true
  const query = searchStr?[{key:'title',value:searchStr}]:[]
await Api.shared().child('nomenclature_items')
.get<NomenclatureItem[]>(query, NomenclatureItem).then(resp =>
{
...
this.loading = false
    ...
    addCategory() {
      if (this.category) {
        const newChild = new DataTree<Data>('', [], [], true, undefined, undefined, this.category.id)
        this.category.children.unshift(newChild)
      }
      else {
        const newChild = new DataTree<Data>('', [], [], true, undefined, undefined, this.nomenclatureTree!.id)
    ...
    return  Api.shared().child('nomenclature').child('categories')
        .get<DataTree<Data>>(query, DataTree<Data>)
    },
    async fetchCategory(id:string) {
      return  Api.shared().child('nomenclature').child('categories',id)
    ...
    async saveCategory(category: DataTree<Data>) {
      if (this.category) {
        this.loading = true        
        Api.shared().child('nomenclature').child('categories')
...
this.loading = true
await Api.shared().child('nomenclature_items',id)
.get<NomenclatureItem>([], NomenclatureItem).then(resp =>
{
...
this.loading = true
await Api.shared().child('nomenclature_items')
.post<CreateNomenclatureItemDto, NomenclatureItem>(this.nomenclatureItem!.toCreate(), NomenclatureItem)
...
this.loading = true
await Api.shared().child('nomenclature_items')
.update<UpdateNomenclatureItemDto, NomenclatureItem>(this.nomenclatureItem!.toUpdate(), NomenclatureItem)
.then(nomenclatureItem => {
this.nomenclatureItems.push(nomenclatureItem)
this.loading = false
})
...
.delete()
.then(nomenclatureItem => {
this.nomenclatureItems = this.nomenclatureItems.filter(e=>e.id! !==id)
this.loading = false
})
},
  }
